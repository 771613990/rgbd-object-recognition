...
def resize_ratio_padding(image, desired_size):
...
def get_camera_param_washington():
    cx_d = 320
    cy_d = 240
    fx_d = 570.3
    fy_d = 570.3
...
def process_depth_image(z, missingMask, C):
...
def get_HHA(C, D, RD):
    missingMask = (RD == 0);
    pc, N, yDir, h, pcRot, NRot = process_depth_image(D * 100, missingMask, C);
